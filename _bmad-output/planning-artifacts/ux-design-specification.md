...
- **8-Category taxonomy (updated 2026-02-22 per brainstorming):** Gallery uses 8 top-level categories with two navigation patterns: **drill-down** (Languages, Essentials) and **direct** (Terminal, Development, Media, Gaming, Power User, Companion Tools). Categories defined in `categories.yaml`, not hardcoded. See brainstorming session 2026-02-20 for full taxonomy. The WPF sidebar shows 8 navigation items. The Astro website is marketing only.
- **NOTE (2026-02-22):** "Suggested" tier removed per brainstorming decision. Layout becomes two-tier: "Your Apps" (detected) and "Other Apps" (gallery). Hot badges replace profile-based suggestions.
